"""
Phase 5 multi-session v3: 4 threads, batch upsert via DELETE+INSERT.
"""
...
LOG = r'C:\Users\zengj\.qclaw\workspace\stock_data_project\_temp\phase5_v3.log'
...
# UPSERT via DELETE + INSERT (for DuckDB which doesn't support INSERT OR IGNORE)
# Step 1: DELETE existing rows for these (ts_code, trade_date) pairs
log('Deleting existing 04-07~04-10 rows...')
conn2 = duckdb.connect(DB)

# Build list of (ts_code, trade_date) pairs to delete
...
conn2.commit()
log(f'  Deleted {len(del_pairs)} existing rows')

# Step 2: Batch INSERT
log(f'Inserting {len(batch_raw)} new rows...')
conn2.executemany("""
