...
try:
    ...
    if not os.path.exists(db):
        ...
        print(f"    字段: {cols}")
        print(f"[2b] DB 含 qfq 字段: {has_qfq_db}")
        print(f"[2c] DB 含 hfq 字段: {has_hfq_db}")

        cur.execute("PRAGMA table_info('daily_bar_raw')")
        raw_cols = [r[1] for r in cur.fetchall()]
        print(f"[2d] DB daily_bar_raw 字段数: {len(raw_cols)}")

        # stock_basic 表
        try:
            cnt = cur.execute("SELECT COUNT(*) FROM stock_basic").fetchone()[0]
        ...
        cnt = conn.execute("SELECT COUNT(*) FROM daily_bar_raw").fetchone()[0]
        print(f"[2g] daily_bar_raw 记录数: {cnt}")

        cnt = conn.execute("SELECT COUNT(*) FROM daily_bar_adjusted").fetchone()[0]
        print(f"[2h] daily_bar_adjusted 记录数: {cnt}")
...
try:
    ...
    if not adj.empty:
        ...
        if has_qfq and adj['qfq_close'].notna().any():
            print(f"     qfq_close 示例: {adj['qfq_close'].dropna().head(3).tolist()}")
        if has_hfq and adj['hfq_close'].notna().any():
            print(f"     hfq_close 示例: {adj['hfq_close'].dropna().head(3).tolist()}")
