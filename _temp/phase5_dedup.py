"""
Phase 5 DEDUP FIX: Deduplicate batch_raw/batch_adj by (ts_code, trade_date) before INSERT.
"""
...
LOG = r'C:\Users\zengj\.qclaw\workspace\stock_data_project\_temp\phase5_dedup.log'
...
try:
    ...
    # Cleanup
    for pid in [14504, 16492, 8700, 18636, 9580]:
    ...
    log(f'Download: {len(all_results)} raw rows in {dl_time:.0f}s')

    # Sort + pct_chg + DEDUP by (ts_code, trade_date)
    all_results.sort(key=lambda x: (x[0], x[1]))
    prev_closes = {}
    batch_raw = []  # will deduplicate
    batch_adj = []
    now_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    seen_keys = set()  # DEDUP tracker
    ...
    log(f'After dedup: {len(batch_raw)} raw, {len(batch_adj)} adj (removed {len(all_results)-len(batch_raw)} duplicates)')

    # UPSERT
    conn3 = duckdb.connect(DB)
    unique_pairs = list(seen_keys)
    log(f'Deleting {len(unique_pairs)} existing rows...')
    for code, td in unique_pairs:
        conn3.execute("DELETE FROM daily_bar_raw WHERE ts_code = ? AND CAST(trade_date AS VARCHAR) = ?", [code, td])
        conn3.execute("DELETE FROM daily_bar_adjusted WHERE ts_code = ? AND CAST(trade_date AS VARCHAR) = ?", [code, td])
    ...
    total = time.time() - t0
    log(f'=== DONE: {len(batch_raw)} rows in {total:.0f}s ===')
